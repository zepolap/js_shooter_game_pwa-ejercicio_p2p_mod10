...
class Game {
    ...
    constructor() {
        ...
        this.score = 0; // Guarda información de la marcación (inicialmente 0)
    }
    ...
    pauseOrResume() {
        if (this.paused) {
            this.timer = setInterval(() => this.update(), 50);
            GAME_UI.app.classList.remove('paused');
            this.paused = false;
        } else {
            clearInterval(this.timer);
            GAME_UI.app.classList.add('paused');
            this.paused = true;
        }
    }
    /**
    ...
    endGame() {
        ...
        if ( this.player.lives > 0 && this.score > OPPONENT_SCORE) {
            imagen = YOU_WIN_PICTURE;
        }
        else {
            imagen = GAME_OVER_PICTURE;
        }
        
        let gameOver = new Entity(this, this.width / 2, "auto", this.width / 4, this.height / 4, 0, imagen);
        gameOver.render();
    ...
    render() {
        ...
        // se actualiza marcadores Score y Lives
